...
> Mutation scoring is part of susgrade's design and specification. The current release ships the complexity engine and the full application around it; the mutation engine is the next stage of the build — see [Roadmap & Vision](#roadmap--vision).
...
| ✅ | **REST API** — `POST /analyze/complexity` and `GET /health`, built on FastAPI + Pydantic | **Implemented** |
...
| ◆ | **Mutation-testing engine** — inject mutants and compute the mutation score | Planned |
...
The complexity engine and the application around it are complete. The path ahead builds toward the full test-effectiveness vision described in [The Core Idea](#the-core-idea):

- **Mutation-testing engine** — generate mutants for a target module, run its test suite against each, and compute a mutation score.
...
- A **live analyzer** that mirrors the engine's logic in the browser for instant, offline feedback.
...
Capabilities that are still on the [roadmap](#roadmap--vision) are surfaced in the UI as clearly-labelled "coming soon" states, so the interface communicates the full vision without overstating what ships today.
